...
void ConnectModel::add(int column)
{
    ...
    {
        emit({column, 6 - static_cast<int>(board[column].size())}, player);
    }
    player = !player;
}
...
void ConnectModel::remove(int column)
{
    ...
    {
        emit({column, 6 - static_cast<int>(board[column].size())});
    }
    board[column].pop();
    player = !player;
}
        ...
        for (int c = 0; c < cols; ++c) {
            if (matchValue(player, board, r, c)) {
                ...
                if (matchValue(player, board, r, c + 1) && matchValue(player, board, r, c + 2) && matchValue(player, board, r, c + 3)) {
                    for (int i = 0; i < 4; ++i) winningPositions.push_back({c + i, 5 - r});
                ...
                {
                    for (int i = 0; i < 4; ++i) winningPositions.push_back({c, 5 - (r + i)});
                    return winningPositions;
                }
                ...
                if (matchValue(player, board, r + 1, c + 1) && matchValue(player, board, r + 2, c + 2) && matchValue(player, board, r + 3, c + 3)) {
                    for (int i = 0; i < 4; ++i) winningPositions.push_back({c + i, 5 - (r + i)});
                    return winningPositions;
                }
