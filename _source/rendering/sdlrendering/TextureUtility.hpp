...
inline SDL_Texture* CreateTextureFromBMP(SDL_Renderer* renderer, const char* filePath) {
    // SDL3 now has LoadBMP_RW instead of SDL_LoadBMP
    SDL_Surface* surface = SDL_LoadBMP(filePath);
    if (!surface) {
        SDL_Log("Unable to load BMP %s! SDL Error: %s\n", filePath, SDL_GetError());
        return nullptr;
    }
...
inline SDL_Texture* CreateGridTexture(SDL_Renderer* renderer, SDL_Texture* tile, int nx, int ny) 
{
    ...
    for (int i = 0; i < nx; ++i) {
        for (int j = 0; j < ny; ++j) {
            ...
            //SDL_FRect srcRect = { i * cellWidth, j * cellHeight, cellWidth, cellHeight };

            // Calculate the destination rectangle for this cell
            SDL_FRect dstRect = { i * w, j * h, w, h };
