...
{
    textures.push_back(LoadBMPTexture(p_->ren, path));
    return textures.size(); // handle is index+1
}

TextureHandle loadBMPTextureWithKey(const char* path, const glm::ivec3& color_key)
{
    textures.push_back(LoadBMPTextureWithKey(p_->ren, path, {color_key.r, color_key.g, color_key.b}));
    return textures.size(); // handle is index+1
}
